...
const DataStructures3D = () => {
  ...
  const renderStructure = (structure: any, index: number) => {
    const motionProps = {
      ...
      transition: {
        rotateZ: { duration: 20 + index * 2, repeat: Infinity, ease: "linear" as any },
        scale: {
          duration: 3,
          repeat: Infinity,
          ...
          <motion.div key={structure.name} {...motionProps}>
            <div className="flex gap-1">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="w-3 h-3 bg-green-500 rounded border border-green-400"
                />
              ))}
            </div>
          </motion.div>
          ...
          <motion.div key={structure.name} {...motionProps}>
            <div className="grid grid-cols-2 gap-1">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="w-3 h-3 bg-purple-500 rounded border border-purple-400"
                />
              ))}
            </div>
          </motion.div>
          ...
          <motion.div key={structure.name} {...motionProps}>
            <div className="flex flex-col-reverse gap-1">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="w-6 h-2 bg-orange-500 rounded border border-orange-400"
                />
              ))}
            </div>
          </motion.div>
          ...
          <motion.div key={structure.name} {...motionProps}>
            <div className="flex gap-1">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="w-2 h-6 bg-cyan-500 rounded border border-cyan-400"
                />
              ))}
            </div>
          </motion.div>
      ...
      {Array.from({ length: 15 }).map((_, i) => (
        <motion.div
          ...
          style={{
            left: `${i * 7}%`,
            background:
              "linear-gradient(0deg, transparent, #8B5CF640, transparent)",
