...
const DramaticRoles = () => {
  ...
  return (
    <motion.div className="relative mb-12" key={currentRole}>
      <motion.div
...
const MegaTechStack = () => {
  ...
  try {
    if (
      skills?.techStack &&
      Array.isArray(skills.techStack) &&
      skills.techStack.length > 0
    ) {
      const validTechStack = skills.techStack.filter(
        (tech) =>
          tech &&
          typeof tech === "object" &&
          tech.name &&
...
const ExplosiveStats = () => {
  ...
  try {
    if (stats?.hero && Array.isArray(stats.hero) && stats.hero.length > 0) {
      const validStats = stats.hero.filter(
        (stat) =>
          stat &&
          typeof stat === "object" &&
          stat.label &&
