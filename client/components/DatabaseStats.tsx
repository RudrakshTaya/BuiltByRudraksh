...
export const DatabaseStats = () => {
  ...
  const tables = [
    {
      ...
      schema: [
        { column: "achievement_id", type: "INT", constraint: "PRIMARY KEY AUTO_INCREMENT" },
        { column: "title", type: "VARCHAR(255)", constraint: "NOT NULL" },
        { column: "category", type: "ENUM('academic','coding','project')", constraint: "NOT NULL" },
      ...
      data: [
        { achievement_id: 1, title: "DSA Problems Solved", category: "coding", score: 250, date_achieved: "2024-01-15", verified: true },
        { achievement_id: 2, title: "CGPA Score", category: "academic", score: 8.7, date_achieved: "2023-12-20", verified: true },
        { achievement_id: 3, title: "Projects Completed", category: "project", score: 15, date_achieved: "2024-01-10", verified: true },
        { achievement_id: 4, title: "Years of Learning", category: "academic", score: 3, date_achieved: "2021-08-01", verified: true },
      ],
    },
    {
      name: "skills",
      ...
      primaryKey: "skill_id",
      schema: [
        { column: "skill_id", type: "INT", constraint: "PRIMARY KEY AUTO_INCREMENT" },
        { column: "name", type: "VARCHAR(100)", constraint: "NOT NULL UNIQUE" },
        { column: "category", type: "VARCHAR(50)", constraint: "NOT NULL" },
        { column: "proficiency", type: "INT", constraint: "CHECK (proficiency BETWEEN 1 AND 100)" },
        { column: "years_experience", type: "DECIMAL(3,1)", constraint: "NULL" },
        { column: "last_used", type: "DATE", constraint: "NOT NULL" },
      ],
      data: [
        { skill_id: 1, name: "React", category: "Frontend", proficiency: 90, years_experience: 2.5, last_used: "2024-01-15" },
        { skill_id: 2, name: "Node.js", category: "Backend", proficiency: 85, years_experience: 2.0, last_used: "2024-01-14" },
        { skill_id: 3, name: "Python", category: "Programming", proficiency: 88, years_experience: 3.0, last_used: "2024-01-12" },
        { skill_id: 4, name: "Java", category: "Programming", proficiency: 82, years_experience: 2.5, last_used: "2024-01-10" },
        { skill_id: 5, name: "MongoDB", category: "Database", proficiency: 80, years_experience: 1.5, last_used: "2024-01-13" },
      ],
    },
    {
      name: "projects",
      ...
      schema: [
        { column: "project_id", type: "INT", constraint: "PRIMARY KEY AUTO_INCREMENT" },
        { column: "name", type: "VARCHAR(255)", constraint: "NOT NULL" },
        { column: "status", type: "ENUM('active','completed','archived')", constraint: "DEFAULT 'active'" },
        { column: "complexity", type: "ENUM('low','medium','high')", constraint: "NOT NULL" },
        { column: "stars", type: "INT", constraint: "DEFAULT 0" },
        { column: "commits", type: "INT", constraint: "DEFAULT 0" },
        { column: "created_at", type: "TIMESTAMP", constraint: "DEFAULT CURRENT_TIMESTAMP" },
      ],
      data: [
        { project_id: 1, name: "E-Commerce Platform", status: "completed", complexity: "high", stars: 42, commits: 156, created_at: "2023-09-15" },
        { project_id: 2, name: "Task Management App", status: "completed", complexity: "medium", stars: 28, commits: 89, created_at: "2023-11-20" },
        { project_id: 3, name: "Weather Dashboard", status: "completed", complexity: "low", stars: 15, commits: 34, created_at: "2023-08-10" },
        { project_id: 4, name: "Chat Application", status: "active", complexity: "medium", stars: 7, commits: 23, created_at: "2024-01-05" },
      ],
    },
  ];
    ...
    setTimeout(() => {
      ...
      if (currentTable) {
        ...
        if (selectedQuery === 0 && activeTable === "achievements") {
          result = result.filter((item: any) => item.score > 50).sort((a: any, b: any) => b.score - a.score);
        } else if (selectedQuery === 1 && activeTable === "skills") {
          result = result.filter((item: any) => item.category === "Programming").sort((a: any, b: any) => b.proficiency - a.proficiency).slice(0, 3);
        }
  ...
  const getTypeColor = (type: string) => {
    ...
    if (type.includes("DATE") || type.includes("TIMESTAMP")) return "text-[#dcdcaa]";
    return "text-[#d4d4d4]";
  };
            ...
            <div className="bg-[#252526] border-r border-[#3c3c3c]">
              <div className="p-4">
                ...
                <div className="mt-6 p-3 bg-[#1e1e1e] border border-[#3c3c3c] rounded">
                  <h4 className="text-[#d4d4d4] font-medium text-sm mb-2">Table Info</h4>
            ...
            <div className="bg-[#1e1e1e] border-r border-[#3c3c3c]">
              <div className="p-4">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    onClick={() => setSelectedQuery((selectedQuery + 1) % queries.length)}
                ...
                {/* Schema Table */}
                <div className="bg-[#0c0c0c] border border-[#3c3c3c] rounded overflow-hidden">
                  <table className="w-full text-xs">
                    <thead className="bg-[#2d2d2d]">
                      <tr>
                        <th className="px-3 py-2 text-left text-[#d4d4d4] font-medium">Column</th>
                        <th className="px-3 py-2 text-left text-[#d4d4d4] font-medium">Type</th>
                      </tr>
                    </thead>
                    <tbody>
                      {currentTable.schema.map((column, index) => (
                        ...
                        >
                          <td className="px-3 py-2">
                            <div className="flex items-center gap-2">
                              <span className="text-[#dcdcaa]">{column.column}</span>
                              {column.constraint.includes("PRIMARY KEY") && (
                                <span className="bg-[#f9c23c] text-black text-xs px-1 rounded">PK</span>
                              )}
                            </div>
                          </td>
                          <td className="px-3 py-2">
              ...
              <div className="p-4">
                ...
                <div className="bg-[#0c0c0c] border border-[#3c3c3c] rounded mb-4">
                  <div className="bg-[#2d2d2d] border-b border-[#3c3c3c] px-3 py-2">
                    <span className="text-[#d4d4d4] text-xs font-mono">query.sql</span>
                  </div>
                  ...
                  <div className="max-h-64 overflow-auto">
                    {isQuerying ? (
                      <div className="p-8 text-center">
                        ...
                        <div className="text-[#969696] text-sm">Executing query...</div>
                      </div>
                    ) : queryResult.length > 0 ? (
                      <table className="w-full text-xs">
                        <thead className="bg-[#252526] sticky top-0">
                          <tr>
                            {Object.keys(queryResult[0]).map((key) => (
                              <th key={key} className="px-3 py-2 text-left text-[#d4d4d4] font-medium">
                                {key}
                              </th>
                            ))}
                          </tr>
                          ...
                          {queryResult.map((row, index) => (
                            ...
                            >
                              {Object.values(row).map((value: any, cellIndex) => (
                                <td key={cellIndex} className="px-3 py-2 text-[#cccccc]">
