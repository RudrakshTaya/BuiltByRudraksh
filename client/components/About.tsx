...
const SkillBar = ({
  ...
  skill: (typeof skills.programming)[0];
  index: number;
  delay?: number;
}) => {
  return (
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-2 gap-2">
        <div className="flex items-center gap-2">
          <div
            className={`w-3 h-3 ${skill.color} rounded-full flex-shrink-0`}
          />
        ...
        <span className="text-muted-foreground text-sm font-mono self-start md:self-auto">
          {skill.level}%
        </span>
      </div>
...
const StatCard = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 ${stat.bgColor}/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl`}
      />

      <div
        className={`relative z-10 w-12 h-12 md:w-16 md:h-16 ${stat.bgColor} rounded-full flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300`}
      >
...
const AcademicCard = ({
  ...
  highlight: (typeof academicHighlights)[0];
  index: number;
}) => {
  return (
    <motion.div
    ...
    >
      <div className="flex flex-col md:flex-row items-start gap-4">
        <div className="text-2xl md:text-3xl self-center md:self-start">
          {highlight.icon}
        </div>
          ...
          <div className="glass p-4 md:p-8 rounded-2xl border border-neon-blue/30 hover:border-neon-blue/50 transition-all duration-300 max-w-4xl mx-auto">
            <div className="flex flex-col md:flex-row items-center justify-center gap-2 md:gap-3 mb-4">
              ...
              <h3 className="text-lg md:text-2xl font-bold text-white text-center">
                Ready for Software Engineering Challenges
              </h3>
            </div>
            <p className="text-muted-foreground mb-6 text-sm md:text-lg">
              With strong foundations in computer science theory and hands-on
              experience in full-stack development, I'm prepared to tackle
              complex software engineering problems. From optimizing algorithms
